# EEG

## 10-20 Electrode Placement System

The 10-20 system is a widely used method in electroencephalography (EEG) for placing electrodes on the scalp. It is designed to provide standardized and reproducible locations, with electrode positions based on the relationship to the skull size. The system's name refers to the fact that the distances between adjacent electrodes are either 10% or 20% of the total front-to-back or right-to-left measurement of the head. This system ensures consistent and accurate data collection across different individuals and studies.

<div align="center">
  <img src="https://upload.wikimedia.org/wikipedia/commons/7/70/21_electrodes_of_International_10-20_system_for_EEG.svg" alt="10-20 Electrode Placement System">
</div>


## P300 speller

A **P300 speller** is a brain-computer interface (BCI) system that enables communication for individuals with severe motor impairments, such as those with locked-in syndrome. It is based on the P300 event-related potential (ERP), a measurable brainwave that occurs approximately 300 milliseconds after a person recognizes a stimulus (hence the name "P300").

how it typically works:

1. **Grid Display**: The user is presented with a grid of letters or symbols on a screen, and the rows and columns of the grid flash in random order.
   
2. **Target Recognition**: The user is asked to focus on the character they want to select, and when the row or column containing that character flashes, it triggers a P300 response in their brain.

3. **Signal Detection**: Electroencephalography (EEG) sensors record brain signals. The system detects the P300 signal corresponding to the flashed row or column, and after several repetitions, it identifies the desired character.

4. **Character Selection**: Based on the detected P300 responses, the system selects and displays the character the user is focusing on.

## Data Set
[![BCI Competition IV](https://www.bbci.de/competition/iv/wct.png)](http://www.bbci.de/competition/iv/)

[![BNCI](http://bnci-horizon-2020.eu/images/logo_BNCI.png)](http://bnci-horizon-2020.eu/database/data-sets)

[GitHub](https://github.com/MikeWangWZHL/EEG-To-Text)

[GitHub](https://github.com/andreafresco/P300-Speller/tree/main)

[GitHub](https://github.com/vtr0n/p300-speller?tab=readme-ov-file)

## What device will we use?

[Performance of the Emotiv Epoc headset for P300-based applications](https://www.ncbi.nlm.nih.gov/pmc/articles/PMC3710229/)

[EMOTIV TUTORIAL](https://www.youtube.com/playlist?list=PLAf9lWfzX4jAM700MQBwGAoR_YSlWUg4b)

[![epoc](https://www.emotiv.com/cdn/shop/files/Emotiv-logo-white.png?v=1695617152)](https://www.emotiv.com/products/epoc-x)


## Youtube

[P300 Speller with V-AMP y Open ViBE](https://www.youtube.com/watch?v=z-KZWyEJPbc)

## 🔗 Links

[![facebook](https://img.shields.io/badge/facebook-0077B5?style=for-the-badge&logo=facebook&logoColor=white)](https://www.facebook.com/qossay.rida?mibextid=2JQ9oc)

[![Whatsapp](https://img.shields.io/badge/Whatsapp-25D366?style=for-the-badge&logo=Whatsapp&logoColor=white)](https://wa.me/+972598592423)

[![linkedin](https://img.shields.io/badge/linkedin-0077B5?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/qossay-rida-3aa3b81a1?utm_source=share&utm_campaign=share_via&utm_content=profile&utm_medium=android_app)

[![twitter](https://img.shields.io/badge/twitter-1DA1F2?style=for-the-badge&logo=twitter&logoColor=white)](https://twitter.com/qossayrida)
